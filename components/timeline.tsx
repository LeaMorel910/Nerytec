...
export function Timeline({
  ...
  subtitle = "Plus de 20 ans d'expertise à votre service",
  events = [
    {
      year: "2005",
  ...
  const iconForIndex = (index: number) => (index === 0 ? Calendar : index === 1 ? Target : index === 2 ? Award : Users)

  return (
    <section id="notre-parcours" className="py-20 bg-white mobile-section">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16 mobile-header">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4 mobile-title">{title}</h2>
          <p className="text-xl text-gray-600 mobile-subtitle">{subtitle}</p>
        </div>
          ...
          {events.map((event, index) => (
            ...
            >
              <div className="w-16 h-16 bg-[#0078BE] rounded-full flex items-center justify-center z-10 mobile-icon-container">
                {React.createElement(iconForIndex(index), { className: "w-8 h-8 text-white mobile-icon" })}
              </div>
