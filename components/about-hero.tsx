...
export function AboutHero({
  ...
  subtitle = "Plus de 20 ans d'expertise dans le recrutement de talents commerciaux et managériaux",
  pills = ["Expertise humaine", "Approche personnalisée", "Résultats prouvés"],
}: AboutHeroProps) {
      ...
      <div ref={ref} className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        >
          ...
          <span className="bg-[#0078BE] bg-clip-text text-transparent">
            {title.split(" ").slice(1).join(" ") || "NERYTEC"}
          </span>
        </h1>
        ...
        >
          {(pills || []).slice(0, 3).map((pill, i) => {
            const Icon = i === 0 ? Users : i === 1 ? Target : Award
            return (
